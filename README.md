...
1) `/start` - стартовая команда. После нажатии на кнопку "Старт" будет отправлено описание бота, если пользователь не зарегестрирован, иначе - отправиться сообщение, что пользователь зарегестрирован, и предложит команду `/help`
2) `/help` - команда содержащая список доступных команд в боте
3) `/tarrif` - выдает информацию от доступных тарифах
4) `/count_req` - позволяет узнать количество оставшихся запросов
5) `/ask_info` - позволяет задать вопрос в нейросеть YandexGPT по теме "ЕГЭ по информатике"
6) `/ask_all` - позволяет задать вопрос в нейросеть YandexGPT по любому вопросу ЕГЭ
7) `/support` - отправить сообщение в поддержку бота. 
